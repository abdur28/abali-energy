...
const ServicesPage = () => {
    return (
        <div>
            <Header text="Our Services" backgroundImage="https://media.istockphoto.com/id/1807044866/photo/engineer-survey-team-wear-uniform-and-helmet-stand-workplace-checking-blueprint-project-radio.jpg?s=612x612&w=0&k=20&c=94CwjoGL13_ywO8TzmIDrZkzn0jtIUAUtTIY5wEhVkw="/>
            <div className="w-full h-full flex flex-col gap-20 lg:gap-10 py-20">
                {services.map((service, index) => (
                    <SingleService key={index} id={service.id.toString()} image={service.image} name={service.name} description={service.description} />
                ))}
            </div>
        </div>
    )
